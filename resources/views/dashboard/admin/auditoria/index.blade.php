...
@section('content')
    ...
    <div class="container mx-auto py-5">
        ...
        <div class="row justify-content-end align-items-end mb-2">
            ...
            <div class="col-md-4">
                <form action="{{ route('administracion.auditoria.index') }}" method="GET">
                    <div class="input-group shadow-sm">
                        <span class="input-group-text bg-white border-end-0 text-muted">
                            <i data-feather="search" class="icon-sm"></i>
                        </span>

                        <input type="text" name="busqueda" class="form-control border-start-0 border-end-0 shadow-none"
                            placeholder="Buscar por usuario, módulo o IP..." value="{{ request('busqueda') }}">

                        @if (request('busqueda'))
                            <a href="{{ route('administracion.auditoria.index') }}"
                        ...
                        <tbody>
                            @forelse($logs as $log)
                                <tr class="clickable-row"
                                    onclick="window.location='{{ route('administracion.auditoria.show', $log->id_auditoria) }}'">
                                    <td class="ps-4">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-sm me-2 bg-slate-50 border rounded-circle d-flex align-items-center justify-content-center"
                                    ...
                                    <td class="text-center">
                                        @if ($log->accion == 'CREAR')
                                            <span
                                                class="badge bg-success-subtle text-success border border-success-subtle px-3">CREAR</span>
                                        @elseif($log->accion == 'MODIFICAR' || $log->accion == 'ACTUALIZAR')
                                            <span
                                                class="badge bg-warning-subtle text-warning-emphasis border border-warning-subtle px-3">MODIFICAR</span>
                                        @else
                                            <span
                                                class="badge bg-danger-subtle text-danger border border-danger-subtle px-3">ELIMINAR</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td class="pe-4 text-end">
                                        <a href="{{ route('administracion.auditoria.show', $log->id_auditoria) }}"
