...
class AlineacionController extends Controller
{
    ...
    public function index($organizacionId)
    {
        ...
        // Cargamos objetivos con su ODS para cualquier lógica extra
        $objetivosNacionales = ObjetivoNacional::with('ods')->get();

        // Cargamos todas las metas para el select dinámico
        $metasNacionales = MetaPnd::all();

        // Recuperamos los ODS ordenados
        $ods = Ods::orderBy('numero', 'asc')->get();

        // Objetivos estrategicos de la organización
        //  Usamos plural para mayor claridad y coherencia
        $objetivosEstrategicos = ObjetivoEstrategico::where('id_organizacion', $organizacionId)->get();

        // Alineaciones existentes
        $alineaciones = AlineacionEstrategica::where('organizacion_id', $organizacionId) // la columna  organizacion_id o id_organizacion
            ->with(['objetivoEstrategico', 'objetivoNacional', 'ods', 'metaPnd']) // Agregamos metaPnd para verla en la tabla
            ->get();
        ...
        return view('dashboard.estrategico.alineacion.gestionar', compact(
            ...
            'metasNacionales',
            'alineaciones',
            'objetivosEstrategicos', // Coincide con la variable de arriba
            'ods'
        ));
    }
    ...
    public function perfil($id)
    {
        ...
        // 4. Retornamos
        return view('dashboard.estrategico.organizaciones.perfil', [
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'objetivo_nacional_id'    => 'required',
            'ods_id'                  => 'required',
            'id_meta_pnd'             => 'required', // Nombre del input HTML
        ]);
